...
void driveCallback ( const geometry_msgs::Twist&  twistMsg )
{
  ...
  if (twistMsg.linear.x >= 0.5) {
    escCommand = (int)fmap(twistMsg.linear.x, 0.0, 1.0, 92.0, maxThrottle) ;
  } else {
...
//{
// // escCommand = (int)fmap(twistMsg.linear.x, 0.0, 1.0, 0.0, 180.0) ;
//  if (twistMsg.linear.x == 100)
//  {
//    //steeringServo.write(120);
//    electronicSpeedController.write(100);
//  }
//  else{
//    electronicSpeedController.write(92);
//  }
//}
...
void setup(){
  pinMode(13, OUTPUT);
  Serial.begin(115200) ;
  nodeHandle.initNode();    // This can be useful for debugging purposes
  nodeHandle.advertise(chatter);  // Subscribe to the steering and throttle messages
  nodeHandle.subscribe(driveSubscriber) ;   // Attach the servos to actual pins
  steeringServo.attach(10); // Steering servo is attached to pin 9
